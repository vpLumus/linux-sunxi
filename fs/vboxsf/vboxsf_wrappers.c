...
int vboxsf_query_mappings(struct shfl_mapping mappings[], u32 *mappings_len)
{
	...
	parms.flags.type = VMMDevHGCMParmType_32bit;
	parms.flags.u.value32 = SHFL_MF_UCS2;

	parms.number_of_mappings.type = VMMDevHGCMParmType_32bit;
	parms.number_of_mappings.u.value32 = *mappings_len;

	parms.mappings.type = VMMDevHGCMParmType_LinAddr_Kernel;
...
int vboxsf_query_mapname(SHFLROOT root, struct shfl_string *string, u32 size)
{
	...
	parms.root.type = VMMDevHGCMParmType_32bit;
	parms.root.u.value32 = root;

	parms.name.type = VMMDevHGCMParmType_LinAddr_Kernel;
...
int vboxsf_map_folder(struct shfl_string *folder_name, SHFLROOT *root)
{
	...
	parms.path.type = VMMDevHGCMParmType_LinAddr_Kernel;
	...
	parms.delimiter.type = VMMDevHGCMParmType_32bit;
	parms.delimiter.u.value32 = '/';

	parms.case_sensitive.type = VMMDevHGCMParmType_32bit;
		  ...
		  struct shfl_createparms *create_parms)
{
	...
	parms.parms.type = VMMDevHGCMParmType_LinAddr_Kernel;
...
int vboxsf_close(SHFLROOT root, SHFLHANDLE file)
{
	...
	parms.handle.type = VMMDevHGCMParmType_64bit;
...
int vboxsf_remove(SHFLROOT root, struct shfl_string *parsed_path, u32 flags)
{
	...
	parms.path.type = VMMDevHGCMParmType_LinAddr_Kernel_In;
	parms.path.u.pointer.size = shfl_string_buf_size(parsed_path);
	parms.path.u.pointer.u.linear_addr = (uintptr_t)parsed_path;
		...
		u32 *buf_len, u8 *buf)
{
	...
	parms.handle.type = VMMDevHGCMParmType_64bit;
	parms.handle.u.value64 = file;
	parms.offset.type = VMMDevHGCMParmType_64bit;
	parms.offset.u.value64 = offset;
	parms.cb.type = VMMDevHGCMParmType_32bit;
	parms.cb.u.value32 = *buf_len;
	parms.buffer.type = VMMDevHGCMParmType_LinAddr_Kernel_Out;
		 ...
		 u32 *buf_len, u8 *buf)
{
	...
	parms.cb.type = VMMDevHGCMParmType_32bit;
	parms.cb.u.value32 = *buf_len;
	parms.buffer.type = VMMDevHGCMParmType_LinAddr_Kernel_In;
	parms.buffer.u.pointer.size = *buf_len;
	parms.buffer.u.pointer.u.linear_addr = (uintptr_t)buf;
		   ...
		   u32 *buf_len, struct shfl_dirinfo *buf, u32 *file_count)
{
	...
	parms.resume_point.type = VMMDevHGCMParmType_32bit;
	parms.resume_point.u.value32 = index;
	parms.file_count.type = VMMDevHGCMParmType_32bit;
		  ...
		  u32 *buf_len, void *buf)
{
	...
	parms.cb.type = VMMDevHGCMParmType_32bit;
	parms.cb.u.value32 = *buf_len;
	parms.info.type = VMMDevHGCMParmType_LinAddr_Kernel;
		...
		u64 size, u32 lock)
{
	...
	parms.offset.type = VMMDevHGCMParmType_64bit;
	parms.offset.u.value64 = offset;
	parms.length.type = VMMDevHGCMParmType_64bit;
