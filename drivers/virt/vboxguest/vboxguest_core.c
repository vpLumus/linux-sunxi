...
static void vbg_guest_mappings_init(struct vbg_dev *gdev)
{
	...
	for (i = 0; i < GUEST_MAPPINGS_TRIES; i++) {
		...
		req->header.request_type = VMMDevReq_SetHypervisorInfo;
...
/**
 * Handle VBG_IOCTL_CHANGE_FILTER_MASK.
 *
...
/**
 * Handle VBG_IOCTL_CHANGE_GUEST_CAPABILITIES.
 *
 * @returns VBox status code
 * @param   gdev	The Guest extension device.
 * @param   session	The session.
 ...
 * @param   data      The i/o data buffer, minimum sizeof(struct vbg_ioctl_hdr)
 */
